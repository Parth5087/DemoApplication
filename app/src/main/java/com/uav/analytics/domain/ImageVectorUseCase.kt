    ...
    suspend fun getNearestPersonName(
        ...
        for (result in faceDetectionResult) {
            ...
            if (recognitionResult != null) {
                ...
                Log.d("CosineDistance", "idsss ${ids}: $distance")
                ...
                lastDetectionTime=imagesVectorDB.getCreatedTimeByPerson(recognitionResult.personID)
                timeDiff=(currentTime - lastDetectionTime)/60000 // This gives positive seconds
...
//                }
                ...
                Log.d("timeDiff", "Time difference: $timeDiff minutes")
                Log.d("timeDiff1", "Last detection: $lastDetectionTime, Current: $currentTime" +
                        ", PersonID: ${recognitionResult.personID} " +
                        ", isOLD: ${imagesVectorDB.getIsOldPersonByPerson(recognitionResult.personID)}")

            }


            val personName = if (recognitionResult == null || distance < similarityThreshold || (timeDiff >= 3 && !imagesVectorDB.getIsOldPersonByPerson(recognitionResult.personID)) ) {
                ...
                //if same person in frame than to change createdAt Time
                imagesVectorDB.removeDataByPersonId(recognitionResult.personID)
                imagesVectorDB.addFaceImageRecord(FaceImageRecord(
                    personID = recognitionResult.personID,
                    personName = recognitionResult.personName,
